Brahma("brahma.landing", function() {
	...
	Brahma.applet("landing", {
		...
		buildControlMenu : function() {
			...
			.with(function() {
				var ul = this;
				$.each(landing._screens, function(id, data) {
					var name = data.name;
					$(ul).put($("<li />"))
					.css({
						'padding': "4px 12px",
		...
		screens : function(scrs) {
			...
			if (scrs instanceof Array) {
				...
				var q = 0;
				for (var i in scrs) {
					var name = "string"==typeof scrs[i].name ? scrs[i].name : i;
					ds[q] = {

						'name': name,
						'filetype': "string"==typeof scrs[i].filetype ? scrs[i].filetype : this.config.filetype,
						'enabled': (typeof "object" == typeof settings.screens[name] && settings.screens[name].enabled) ? true : ("boolean"==typeof scrs[i].enabled ? scrs[i].enabled : defaultEnabled)
					}
					q++;
				}
			};
		...
		build : function(callback) {
			...
			switch (this.config.renderMode) {
				case 'frontend':
					...
					$.each(this._screens, function(id, data) {
						...
						var screen = $(wrapper).put($("<section />", {
							id: 'screen_'+data.name+'-'+id,
							"class": "screen "+data.name
						}));
				...
				default:
					// Include
					$.each(this._screens, function(id, data) {
						applet._screens[id].element = $(wrapper).find('#screen_'+data.name+'-'+id);
					});
					applet.loaded();
				break;
			}
