...
(function(define) {
	...
	*/
	...
	window.Brahma.api.max = function(values) {
	...
	window.Brahma.api.isLocalStorageAvailable = function() {
	  try {
	...
	window.Brahma.api.percentEq = function(value, quantity) {
	...
	window.Brahma.widget = window.Brahma.ext.widget = function() {
		if (this === window || typeof this == 'function') {
			...
			var widget = {
				...
				privatize : function() {
					if ("object" != typeof $(this.selector)[0].widgets)
					Brahma(this.selector)[0].widgets = {};
					Brahma(this.selector)[0].widgets[this.name] = this;
				},
				_stop : function() {
					// Clear intervals
					if (var i in this.privates.intervals) {
